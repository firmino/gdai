...
class PyMuPDFExtractor(DocumentExtractor):
    ...
    def extract_document_data(self, document_path: str) -> dict:
        ...
        print(f"Extracting document {doc_name} with PyMuPDF...")
    ...
    def _join_paragraphs_between_pages(self, text_data: list) -> list:
        """
        ...
        for i in range(len(text_data) - 1):
            ...
            text_data[i] = current_text 
            text_data[i + 1]["text"] = next_text
        
        return text_data
        

    def _extract_raw_text(self, pdf_document) -> list:
        """
        ...
        for page_num, page in enumerate(pdf_document):       
            ...
            if text:  # Only add if there's actual text content
                text_data.append({
                    "text": text,  # Replace newlines with Docling format
                    "prov": [{"page_no": page_num + 1}]  # Match Docling format
                })
    ...
    def _extract_text(self, text_result) -> list[Text]:
        ...
        """
        # Extract text from result to a dictionary
